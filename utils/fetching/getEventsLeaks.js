...
export const obtenerTodo = async (horaInicio, horaFinal) => {
  ...
  const responseTodas = await fetch(
    `https://api-v2.pasalo.pro/api/v2/community-tasks/pins?startDate=${horaInicio}&taskType=3400&transfer=0&endDate=${horaFinalConMargenString}&customerId=593&communitiesIds[]=787&byCreation=`,
    {
      headers: {
...
export const obtenerTodoPorFuenteDeIngreso = async (horaInicio, horaFinal) => {
  ...
  const arrayPromisesResponses = arrayFuentesDeIngreso.map((parametro) => {
    return fetch(
      `https://api-v2.pasalo.pro/api/v2/community-tasks/pins?startDate=${horaInicio}&taskType=3400&transfer=0&customs[56][0]=${parametro}&customerId=593&communitiesIds[]=787&endDate=${horaFinalConMargenString}&byCreation=`,
